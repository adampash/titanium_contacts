...
#define TAG "com.adampash.contactSearch"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> ContactsSearch_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("ContactsSearch.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ContactsSearchBindings::lookupGeneratedInit(
...
static void ContactsSearch_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(ContactsSearch_getBinding)->GetFunction());
}

static void ContactsSearch_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ContactsSearchBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ContactsSearchBinding = {
	"com.adampash.contactSearch",
	ContactsSearch_init,
	ContactsSearch_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_adampash_contactSearch_ContactsSearchBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.adampash.contactSearch", &ContactsSearchBinding);
	titanium::KrollBindings::addExternalLookup(&(::ContactsSearchBindings::lookupGeneratedInit));
}
