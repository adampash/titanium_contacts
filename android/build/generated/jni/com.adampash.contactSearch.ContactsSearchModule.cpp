...
#define TAG "ContactsSearchModule"
...
Persistent<FunctionTemplate> ContactsSearchModule::proxyTemplate = Persistent<FunctionTemplate>();
jclass ContactsSearchModule::javaClass = NULL;

ContactsSearchModule::ContactsSearchModule(jobject javaObject) : titanium::Proxy(javaObject)
{
}

void ContactsSearchModule::bindProxy(Handle<Object> exports)
{
	...
	Handle<String> nameSymbol = String::NewSymbol("ContactsSearch");
...
void ContactsSearchModule::dispose()
{
	LOGD(TAG, "dispose()");
	if (!proxyTemplate.IsEmpty()) {
...
Handle<FunctionTemplate> ContactsSearchModule::getProxyTemplate()
{
	if (!proxyTemplate.IsEmpty()) {
	...
	javaClass = titanium::JNIUtil::findClass("com/adampash/contactSearch/ContactsSearchModule");
	HandleScope scope;
...
, javaClass, nameSymbol);
	...
	proxyTemplate->Set(titanium::Proxy::inheritSymbol,
		FunctionTemplate::New(titanium::Proxy::inherit<ContactsSearchModule>)->GetFunction());
	...
	instanceTemplate->SetAccessor(String::NewSymbol("exampleProp"),
			ContactsSearchModule::getter_exampleProp
			, ContactsSearchModule::setter_exampleProp
...
// Methods --------------------------------------------------------------------
Handle<Value> ContactsSearchModule::getAllContacts(const Arguments& args)
{
	LOGD(TAG, "getAllContacts()");
	HandleScope scope;
	...
	if (!methodID) {
		methodID = env->GetMethodID(ContactsSearchModule::javaClass, "getAllContacts", "(Ljava/lang/String;)[Ljava/util/HashMap;");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getAllContacts' with signature '(Ljava/lang/String;)[Ljava/util/HashMap;'";
			LOGE(TAG, error);
...
Handle<Value> ContactsSearchModule::example(const Arguments& args)
{
	LOGD(TAG, "example()");
	HandleScope scope;
...
Handle<Value> ContactsSearchModule::getter_exampleProp(Local<String> property, const AccessorInfo& info)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ContactsSearchModule::javaClass, "getExampleProp", "()Ljava/lang/String;");
		if (!methodID) {
...
void ContactsSearchModule::setter_exampleProp(Local<String> property, Local<Value> value, const AccessorInfo& info)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ContactsSearchModule::javaClass, "setExampleProp", "(Ljava/lang/String;)V");
		if (!methodID) {
